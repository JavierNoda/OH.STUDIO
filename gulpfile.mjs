...
function css() {
  console.log('Compilando Sass...'); // Mensaje de depuración
  return src('./src/scss/app.scss') // ⚠️ Asegúrate de que esta ruta es correcta
    .pipe(sass()) // Compilar y manejar errores
    .pipe(dest('./build/css')); // Guardar en la carpeta de salida
}

function minificarcss() {
  return src('./src/scss/app.scss')
    .pipe(sass({ outputStyle: 'compressed' }).on('error', sass.logError)) // Minifica el CSS
    .pipe(dest('./build/css')); // Guarda en la carpeta de salida
}

function watchArchivos() {
  // watch('./src/scss/*.scss', css); // Vigilar cambios en los archivos Sass
  watch('./src/scss/**/*.scss', css); // Vigilar cambios en los archivos Sass
}
...
// exports.default = series(css, watchArchivos); // Ejecuta la tarea por defecto
